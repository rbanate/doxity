...
export default function (args) {
  ...
  const tmpTarget = path.resolve(
    `${process.env.PWD}/${target}/../doxity-tmp-${new Date()}`
  );
  ...
  clearDirectory(absoluteTarget)
    ...
    .then(() => {
      fs.copySync(
        `${tmpTarget}/${fs.readdirSync(tmpTarget)[0]}`,
        absoluteTarget
      );
    ...
    .then(() => {
      ...
      const seq = '⣷⣯⣟⡿⢿⣻⣽⣾'.split('');
      const message =
        'Setting up doxity project with npm install. This may take a while...';
      ...
      npmInstall.on('close', () => {
        ...
        if (fs.existsSync(doxityrcFile)) {
          fs.unlinkSync(doxityrcFile);
        }
